...
describe('sanitizeURI', () => {
  ...
  it('should allow a custom replacement character', () => {
    expect(sanitizeURI('duck\\goose.elephant', { replacement: '-' })).toEqual('duck-goose.elephant')
  })
...
describe('sanitizeSlug', () => {
  ...
  it('should keep valid URI chars (letters digits _ - . ~)', () => {
    expect(sanitizeSlug('This, that-one_or.the~other 123!')).toEqual('This-that-one_or.the~other-123')
  })
  ...
  it('should clean accents and strip non-latin chars in "ascii" mode with `clean_accents` set', () => {
    expect(sanitizeSlug('ěščřžý日本語のタイトル', Map({ encoding: 'ascii', clean_accents: true }))).toEqual('escrzy')
  })
